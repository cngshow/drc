...
Warbler::Config.new do |config|
  ...
  config.jar_name = "#{DRCUtilities::MAVEN_TARGET_DIRECTORY}/#{ENV['RAILS_RELATIVE_URL_ROOT']}"
  ...
  # config.public_html = FileList["public/**/*", "doc/**/*"]
    config.public_html += FileList["version.txt"]
  ...
  maven_files = []
  maven_files << 'target/pom.properties' if File.exists?('target/pom.properties')
  maven_files << 'pom.xml' if File.exists?('pom.xml')
  config.script_files = FileList[*maven_files]
end
